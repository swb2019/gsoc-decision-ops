...
test('campaign and free-play entry, persistence, and reset confirmation', async({page})=>{
  ...
  await page.evaluate(()=>{localStorage.setItem('hourglass-campaign-completions',JSON.stringify(['arc-1-foundations']));localStorage.setItem('hourglass-campaign-unlocks',JSON.stringify(['arc-1-foundations']));});
  await page.reload();
  expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('hourglass-campaign-completions')))).toEqual(['arc-1-foundations']);
  page.once('dialog',dialog=>dialog.dismiss());
  await page.getByRole('button',{name:'Reset campaign progress'}).click();
  expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('hourglass-campaign-completions')))).toEqual(['arc-1-foundations']);
  page.once('dialog',dialog=>dialog.accept());
  await page.getByRole('button',{name:'Reset campaign progress'}).click();
  expect(await page.evaluate(()=>localStorage.getItem('hourglass-campaign-completions'))).toBeNull();
...
test('headset provisioning is opt-in and cancellation returns the switch to idle',async({page})=>{
  let modules=0,models=0,release;
  await page.route('https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.5.1',route=>{
    modules++;
    return route.fulfill({contentType:'text/javascript',headers:{'Access-Control-Allow-Origin':'*'},body:"export const env={}; export async function pipeline(){ await fetch('https://huggingface.co/swb-test/model'); return ()=>{}; }"});
  });
  await page.route('https://huggingface.co/swb-test/model',route=>{
    models++;
    return new Promise(resolve=>{release=async()=>{await route.abort().catch(()=>{});resolve();};});
  });
  await page.goto('/scenarios/access-control-ransomware/');
  expect(modules).toBe(0);expect(models).toBe(0);
  ...
  await toggle.click();await expect.poll(()=>models).toBe(1);
