...
const baseURL = process.env.HOURGLASS_QA_URL || 'http://127.0.0.1:4192/gsoc-decision-ops/';
const results = {
  basis:
    'Five fresh browser contexts per new flagship route, Windows headless Chrome on localhost; software graphics permitted. Descriptive lab observations, not field INP or named integrated-GPU qualification.',
...
for (const review of results.accessibility) assert.deepEqual(review.violations, [], review.screen);
for (const layout of results.layouts) assert.ok(layout.documentWidth <= layout.width + 1);
console.log(
  JSON.stringify(
    {
